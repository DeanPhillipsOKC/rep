...
- [ ] **[human]** Backlog item 15 (per-exercise rest timer with push notification): confirmed working end-to-end on Android, unlocked, including sound (2026-09-24 — the one apparent "no sound" case was the phone's Bluetooth being connected to a car, not a code issue). Still open:
  - [ ] Locked-phone/backgrounded-app delivery: log a set, then lock the screen or switch to another app before the rest timer elapses, and confirm the push still arrives (with vibration). Re-verify specifically after the 2026-09-24 fix in item 28's entry below — a first pass at this found it silently broken (no push, no vibration at all when backgrounded), root-caused to the push only being decided at `addSet` time instead of on the app actually going to background.
