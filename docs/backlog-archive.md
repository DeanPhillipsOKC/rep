...
- Fix set_index race in addSet (item 10): added a `sets_set_index` BEFORE INSERT trigger (`supabase/schema.sql`) that recomputes `set_index` server-side as `max(set_index) + 1` for the target `workout_id`, guarded by a `pg_advisory_xact_lock` keyed on `workout_id` so two concurrent inserts for the same workout can't both read the same max before either commits. Purely additive at the DB level — the client still sends a best-effort `set_index` (now just a not-null placeholder, overwritten by the trigger) so the app behaves identically before the migration is applied and self-corrects once it is, no client redeploy needed. Migration not yet applied to the live database — no DDL execution path available to Claude in this repo, same constraint as item 8's `setup_notes` column; needs the user to run the new statements from `supabase/schema.sql` in the Supabase SQL editor. `npm run build` and the full `npm run test:e2e` suite (including the flaky `e2e/pre-fill.spec.ts` "set position tracks across exercises logged in parallel" spec) pass against the unmodified DB, but since the fix is DB-side and the flake is intermittent (~1 in 4-8 runs), a green run here doesn't confirm the fix — that needs re-verification after the migration lands.
