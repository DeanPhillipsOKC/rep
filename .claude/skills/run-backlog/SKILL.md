...
the user doesn't have to piece together what happened across possibly dozens of `next-item`
iterations from `logs/backlog-runner.log` themselves. This skill does the looping (via
...
`docs/backlog-runner.md` first if you haven't already this session.

## 1. Preconditions

Run `git status --porcelain`. If it prints anything, stop and tell the user the tree isn't clean
instead of stashing or discarding anything on their behalf — the script enforces this too, but
checking first avoids a wasted background run that just fails on the same message.
...
Run `scripts\Run-Backlog.ps1 -Agent Claude`, passing through any `-MaxIterations` /
`-UsageLimitWaitMinutes` the user gave when invoking this skill (otherwise leave the script's
defaults). This can run for a long time — potentially many items, each with a full build+e2e
gate — so start it in the background rather than blocking a single tool call on it, and wait for
the process to actually exit before moving on. Don't poll `logs/backlog-runner.log` mid-run.
...
unless the user asked for one. Structure:
...
- Never retry, adjust flags, or re-run the script on your own initiative because you didn't like
  the outcome; report what happened and stop.
